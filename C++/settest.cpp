...
int main(){
	...
	if(m == 3){
		...
		for(int i=0;i<n;i++){
			...
			while(l<=r){
				int mid = (l+r)/2;
				if(diff(mid,make_pair(cell[i].num[1],cell[i].num[2]))) l = mid + 1;
				else r = mid - 1;
			}
			res = max(res,l);
			if(!S[l].empty()){
				for(auto j = lower_bound(S[l].begin(),S[l].end(),make_pair(cell[i].num[1],cell[i].num[2])); j != S[l].end(); j++){
					if((*j).second > cell[i].num[2]) S[l].erase(j);
					else break;
				}
			}
